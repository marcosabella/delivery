...
export function Landing() {
  ...
  async function loadOrders() {
    ...
    const loadedOrders = (data || []) as CustomerOrder[];
  ...
  async function toggleFavoriteMenuItem(menuItemId: string) {
    ...
    if (error) {
      ...
      setMessage('No se pudo actualizar el producto favorito.');
    }
  }
        ...
        <div className="grid md:grid-cols-[.8fr_1.2fr]">
          ...
          <div className="flex flex-col justify-center p-7 sm:p-10">
            <div className="flex items-start justify-between gap-4">
              ...
              <button type="button" onClick={onToggleFavoriteRestaurant} className={`shrink-0 rounded-full p-2.5 shadow-sm transition ${isFavoriteRestaurant ? 'bg-red-500 text-white hover:bg-red-600' : 'bg-white/10 text-white hover:bg-white/20'}`} aria-label={isFavoriteRestaurant ? 'Quitar restaurante de favoritos' : 'Agregar restaurante a favoritos'} title={isFavoriteRestaurant ? 'Quitar de favoritos' : 'Agregar a favoritos'}>
                <Heart className={`h-5 w-5 ${isFavoriteRestaurant ? 'fill-current' : ''}`} />
              </button>
            </div>
...
function OrdersView({
  ...
  if (selectedOrder) {
    const restaurant = restaurantById.get(selectedOrder.restaurant_id);
    const items = selectedOrder.order_items || [];

    return (
      <section className="mx-auto min-h-[70vh] max-w-4xl px-4 py-10 sm:px-6">
        <div className="space-y-4">
          <button type="button" onClick={onBackToList} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-600 shadow-sm hover:bg-slate-50">
          ...
          <div className="rounded-2xl bg-white shadow-sm">
            <div className="flex flex-col gap-3 border-b p-5 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p className="text-sm font-semibold text-orange-600">Pedido #{selectedOrder.id.slice(0, 8)}</p>
                <h2 className="mt-1 text-xl font-bold">{restaurant?.name || 'Restaurante'}</h2>
                <p className="mt-1 text-sm text-slate-500">
                  {new Date(selectedOrder.created_at).toLocaleString('es-AR')} · {selectedOrder.delivery_method === 'pickup' ? 'Retiro' : 'Delivery'}
                </p>
              </div>
            ...
            <div className="grid gap-3 border-b border-slate-100 p-5 sm:grid-cols-3">
              <div className="rounded-xl border border-slate-200 p-4">
                ...
                <p className="mt-1 text-xl font-black text-orange-600">${Number(selectedOrder.total_amount).toLocaleString('es-AR')}</p>
              </div>
              <div className="rounded-xl border border-slate-200 p-4 sm:col-span-2">
                <p className="text-xs font-semibold uppercase text-slate-500">{selectedOrder.delivery_method === 'pickup' ? 'Retiro' : 'Direccion'}</p>
                <p className="mt-1 text-sm font-medium text-slate-700">{selectedOrder.delivery_address}</p>
              </div>
            </div>

            <div className="p-5">
              <h3 className="font-bold text-slate-900">Detalle del pedido</h3>
              <div className="mt-4 space-y-3">
                {items.length > 0 ? (
                  items.map((item) => (
                    <div key={item.id} className="flex items-start justify-between gap-4 border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                      <div>
                        <p className="font-semibold text-slate-800">{item.quantity}x {item.menu_item?.name || 'Producto'}</p>
                        {item.menu_item?.category && <p className="mt-0.5 text-xs text-slate-500">{item.menu_item.category}</p>}
                        <p className="mt-1 text-xs text-slate-500">${Number(item.unit_price).toLocaleString('es-AR')} c/u</p>
                      </div>
                      <p className="shrink-0 font-bold text-slate-800">${Number(item.subtotal).toLocaleString('es-AR')}</p>
                    </div>
                  ))
                ) : (
                  <p className="rounded-xl bg-slate-50 p-4 text-sm text-slate-500">No hay items cargados para este pedido.</p>
                )}
              </div>
              {selectedOrder.customer_notes && (
                <div className="mt-5 rounded-xl border border-yellow-200 bg-yellow-50 p-4">
                  <p className="text-xs font-semibold uppercase text-yellow-800">Notas</p>
                  <p className="mt-1 text-sm text-yellow-900">{selectedOrder.customer_notes}</p>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="rounded-2xl bg-white shadow-sm">
        ...
        <div className="space-y-3 p-5">
          {orders.length === 0 ? (
            ...
            <button key={order.id} type="button" onClick={() => onSelectOrder(order.id)} className="grid w-full gap-3 rounded-xl border border-slate-200 p-4 text-left transition hover:border-orange-200 hover:bg-orange-50/40 sm:grid-cols-[1fr_auto] sm:items-center">
              <div>
