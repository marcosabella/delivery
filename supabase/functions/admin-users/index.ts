...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (body.action === "create") {
      ...
      const allowedRoles = ["customer", "restaurant_owner", "admin", "driver"];
      ...
      if (callerRole !== "admin" && !(callerRole === "restaurant_owner" && role === "driver")) {
        return json({ error: "User creation access denied" }, 403);
      }
      if (role === "driver" && !restaurantId) {
        return json({ error: "Restaurant is required for drivers" }, 400);
      }
      if (role === "driver" && callerRole === "restaurant_owner") {
      ...
      if (role === "driver") {
        const assignmentResponse = await fetch(`${supabaseUrl}/rest/v1/restaurant_drivers`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${serviceRoleKey}`,
          ...
          body: JSON.stringify({ restaurant_id: restaurantId, driver_id: created.id, is_active: true }),
        });
        ...
        if (!assignmentResponse.ok) {
          ...
          return json({ error: "Could not assign driver to restaurant" }, 500);
        }
      }
